...
router.get("/donation/status", async (req, res) => {
...
router.post("/donation/record", async (req, res) => {
  if (req.body.did) {
    res.render("donate/status", {
      ...
      times: await db.find({ orderid: req.body.did }),
    });
    return;
  }
  res.render("donate/status", {
    path: req.originalUrl,
    times: await db.find({ email: req.body.email }),
  });
});
...
router.post("/success", async (req, res) => {
  responsePayment(req.body).then(
    async (success) => {
      ...
      res.render("donate/donationsuccess.ejs", {
        resultData: "true",
        ret: success,
        rer: dep,
        path: req.originalUrl,
